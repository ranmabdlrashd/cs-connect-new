...
def api_admin_activity_chart():
    ...
    with db_connection() as conn:
        ...
        return jsonify({"success": True, "data": [
            {"date": r['date'].isoformat() if r['date'] else '', "sessions": r['sessions']} 
            for r in rows
        ]})
...
def api_admin_post_notice():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("Error posting admin notice")
...
def api_admin_library_requests():
    ...
    try:
        ...
        logger.exception("Error in admin library requests API")
        return jsonify({"success": True, "data": []}) # Return empty list on error for safety
...
def api_admin_library_stats():
    ...
    with db_connection() as conn:
        metrics = {
            ...
            "issued_books": conn.execute("SELECT COUNT(*) FROM issues WHERE status = 'active'").fetchone()[0] or 0,
            "overdue_books": conn.execute("SELECT COUNT(*) FROM issues WHERE status = 'active' AND due_date < NOW()").fetchone()[0] or 0
        }
...
def api_admin_users():
    ...
    with db_connection() as conn:
        ...
        return jsonify({"success": True, "data": users_list, "total": total, "pages": (total + limit - 1) // limit, "metrics": metrics})
...
def api_admin_results_submissions():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("Results Submissions DB Error")
            return jsonify([])
...
def api_admin_notices_list():
    ...
    with db_connection() as conn:
        try:
            ...
            """
            rows = conn.execute(query).fetchall()
            # Handle date formatting in Python to avoid DATE_FORMAT (MySQL-specific) issues
            results = []
            for r in rows:
                d = dict(r)
            ...
            logger.exception("Notices Fetch Error")
            return jsonify([])
...
def api_admin_events_list():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("Events Fetch Error")
            return jsonify([])
...
def api_admin_placements_drives():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("Placements Fetch Error")
            return jsonify([])
...
def api_admin_courses_subjects():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("Subjects Fetch Error")
            return jsonify([])
...
def api_admin_timetable():
    ...
    with db_connection() as conn:
        try:
            ...
            logger.exception("TT Fetch Error")
    ...
    with db_connection() as conn:
        try:
            ...
            for p in prog_raw:
                ...
                progress.append({
                    "batch": p['batch'].split(' ')[0] if p['batch'] else 'N/A', 
            ...
            logger.exception("Stats Fetch Error")
...
def api_admin_analytics_overview():
    ...
    with db_connection() as conn:
        try:
            ...
            return jsonify({"error": "Failed to fetch analytics"})
...
def api_admin_analytics_cgpa():
    ...
    with db_connection() as conn:
        try:
            ...
            return jsonify({"error": "Failed to fetch CGPA analytics"})
...
def api_admin_reports_generate():
    ...
    logger.info("Bypassed PDF/CSV Logic dynamically generating %s explicitly in %s", payload['type'], payload['format'])
    return jsonify({"success": True})
...
def api_admin_reports_custom():
    ...
    payload = request.json
    logger.info("Constructed Multi-dimensional subsets handling %s mapped onto %s", payload['elements'], payload['format'])
    return jsonify({"success": True})
